        ...
        <section className="bg-white">
            <div className="py-8 px-4 mx-auto max-w-screen-xl lg:py-16 lg:px-6">
                <div className="mx-auto max-w-screen-sm text-center lg:mb-16 mb-8">
                    <h2 className="mb-4 text-3xl lg:text-4xl tracking-tight font-extrabold text-gray-900 dark:text-black">Our Blog</h2>
                    <p className="font-light text-gray-500 sm:text-xl dark:text-gray-400">We use an agile approach to test assumptions and connect with the needs of your audience early and often.</p>
                </div>
                <div className="grid gap-8 lg:grid-cols-2">
                    {blogs?.map((blog) => (
                        <Link to={`/blogs/${blog.id}`} className="p-6 bg-d rounded-lg border border-gray-200 shadow-md dark:border-gray-700"
                            key={blog.id}
                        >
                            ...
                            <div className="flex justify-between items-center">
                                <div className="flex items-center space-x-4">
                                    <img className="w-7 h-7 rounded-full" src={blog.authorImage} alt={`${blog.author} avatar`} />
                                    <span className="font-medium dark:text-black">{blog.author}</span>
                                </div>
